    ...
    <div className="min-h-screen bg-slate-900 text-white flex">
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4">
                {topics.length === 0 ? (
                  ...
                  topics.map((topic) => (
                    <Card key={topic.id} className="bg-slate-800/50 border-slate-700 hover:border-slate-600 transition-colors cursor-pointer" onClick={() => goToTopic(topic)}>
              ...
              <div className="grid gap-3">
                {selectedTopic.subtopics.length === 0 ? (
                  ...
                  selectedTopic.subtopics.map((st) => (
                    <Card key={st.id} className="bg-slate-800/50 border-slate-700 hover:border-slate-600 transition-colors cursor-pointer" onClick={() => goToSubtopic(st)}>
                  ...
                  <Card key={member.id} className="bg-slate-800/50 border-slate-700">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button variant="ghost" size="sm" onClick={() => toggleUserActive(member.id)}>{member.isActive ? <UserX className="h-4 w-4" /> : <User className="h-4 w-4" />}</Button>
                          {member.role !== 'admin' && <Button variant="ghost" size="sm" onClick={() => toggleUserRole(member.id)}><Crown className="h-4 w-4" /></Button>}
                          {member.id !== user.id && <Button variant="ghost" size="sm" className="text-red-400 hover:text-red-300" onClick={() => deleteUser(member.id)}><Trash2 className="h-4 w-4" /></Button>}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
          ...
          {view === 'messages' && (
            <Tabs value={messageTab} onValueChange={(v) => setMessageTab(v as 'received' | 'sent')} className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button size="sm" onClick={handleOpenNewMessage}><Plus className="h-4 w-4 mr-1" />Nuevo Mensaje</Button>
              </div>
              <TabsContent value="received" className="space-y-3">
                {messages.length === 0 ? (
                  <Card className="bg-slate-800/50 border-slate-700"><CardContent className="p-6 text-center text-slate-400">No tienes mensajes.</CardContent></Card>
                ) : (
                  messages.map((msg) => (
                    <Card key={msg.id} className={`bg-slate-800/50 border-slate-700 ${!msg.isRead ? 'border-l-4 border-l-yellow-500' : ''}`}>
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between">
                          ...
                          {!msg.isRead && <Button variant="ghost" size="sm" onClick={() => markAsRead(msg.id)}><CheckCircle className="h-4 w-4" /></Button>}
                        </div>
                      </CardContent>
                    </Card>
                  ))
              ...
              <TabsContent value="sent" className="space-y-3">
                {sentMessages.length === 0 ? (
                  <Card className="bg-slate-800/50 border-slate-700"><CardContent className="p-6 text-center text-slate-400">No has enviado mensajes.</CardContent></Card>
                ) : (
                  sentMessages.map((msg) => (
                    <Card key={msg.id} className="bg-slate-800/50 border-slate-700">
                      <CardContent className="p-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1"><span className="font-medium">{msg.subject}</span></div>
      ...
      <Dialog open={showNewMessage} onOpenChange={setShowNewMessage}>
        <DialogContent className="bg-slate-800 border-slate-700 max-w-lg">
          ...
          <div className="space-y-4 pt-4">
            <select value={newMessageRecipient} onChange={(e) => setNewMessageRecipient(e.target.value)} className="w-full p-2 bg-slate-700 border border-slate-600 rounded text-white">
            ...
            <Button onClick={sendMessage} disabled={!newMessageContent.trim() || !newMessageRecipient}><Send className="h-4 w-4 mr-2" />Enviar</Button>
          </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="bg-slate-800 border-slate-700">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" onClick={() => updateProfile(true)} disabled={loading}><Key className="h-4 w-4 mr-2" />Generar Nueva Clave</Button>
            </div>
            <Button variant="destructive" size="sm" onClick={deleteAccount}>Darse de baja</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!invitedUserKey} onOpenChange={() => setInvitedUserKey(null)}>
        <DialogContent className="bg-slate-800 border-slate-700">
          <DialogHeader><DialogTitle>¡Invitación Creada!</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-4">
            <div className="p-4 bg-slate-700 rounded-lg">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" variant="ghost" onClick={() => copyToClipboard(invitedUserKey?.accessKey || '')}>{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</Button>
              </div>
            </div>
          </div>
        </DialogContent>
      ...
      {newKeyGenerated && (
        <Dialog open={!!newKeyGenerated} onOpenChange={() => setNewKeyGenerated('')}>
          <DialogContent className="bg-slate-800 border-slate-700">
            <DialogHeader><DialogTitle>Nueva Clave Generada</DialogTitle></DialogHeader>
            <div className="p-4 bg-slate-700 rounded-lg">
              <code className="text-yellow-400 text-lg">{newKeyGenerated}</code>
            </div>
            <Button onClick={() => { navigator.clipboard.writeText(newKeyGenerated); setNewKeyGenerated(''); }}>Copiar y cerrar</Button>
          </DialogContent>
        </Dialog>
      )}
      ...
      <Dialog open={!!replyingTo} onOpenChange={(open) => { if (!open) setReplyingTo(null); }}>
        <DialogContent className="bg-slate-800 border-slate-700">
          ...
          <div className="space-y-4 pt-4">
            <div className="p-3 bg-slate-700/50 rounded-lg"><p className="text-sm text-slate-300">{replyingTo?.content}</p></div>
