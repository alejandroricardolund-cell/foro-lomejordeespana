...
// PUT - Editar subtema (creador o admin)
export async function PUT(request: NextRequest) {
  try {
    ...
    if (subtopic.createdBy !== userId && user?.role !== 'admin') {
      return NextResponse.json({ error: 'No tiene permiso para editar este subtema' }, { status: 403 });
    }

    const updatedSubtopic = await db.subtopic.update({
      where: { id },
      data: { name },
      include: {
        creator: { select: { name: true } }
    ...
    console.error('Error editando subtema:', error);
    return NextResponse.json({ error: 'Error interno' }, { status: 500 });
  }
}
